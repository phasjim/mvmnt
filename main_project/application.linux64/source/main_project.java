...
public class main_project extends PApplet {
...
int[] colorPixels = new int[TOTALPIXELS]; // colorPixels will contain the current RGB-color
...
public void compareFrames(){  
  ...
  for (int x = 0; x < WIDTH; x++ ) {
    for (int y = 0; y < HEIGHT; y++ ) {
      ...
      int currPix = currFrame.pixels[loc];     // the current color
      int prevPix = prevFrame.pixels[loc];     // the previous color
...
public void drawCanvas(){    
  for (int loc = 0; loc < framePixels.length; loc++) {
    ...
    if(framePixels[loc] == WHITE && isNotNoise(loc, x, y)) {
      int currColor = colorPixels[loc]; // color pixel at loc
...
public boolean isNotNoise(int loc, int x, int y){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "main_project" };
    if (passedArgs != null) {
